    ...
    return (
        <div className="w-full overflow-x-auto rounded-xl border border-default-200">
            <table className="w-full text-left text-sm">
                <thead className="bg-default-100 text-default-500 uppercase font-medium text-xs">
                    <tr>
                ...
                <tbody className="divide-y divide-default-200">
                    {workItems.map((item) => (
                        <tr key={item.workItem.workitemKey} className="hover:bg-default-50 transition-colors">
                            <td className="px-6 py-4 font-medium text-default-900">{item.workItem.title}</td>
                            ...
                            <td className="px-6 py-4 text-default-600">
                                {item.workItem.dueDate ? (
                                    <span>{format(new Date(item.workItem.dueDate), "d MMM", { locale: es })}</span>
                                ) : (
                            ...
                            <td className="px-6 py-4">
                                <div className="flex items-center justify-end gap-2">
                                    {(userRole === "DEV" || userRole === "DEVOPS" || userRole === "UI_DESIGNER") && item.workItem.ownerId !== currentOwnerId ? (
                                        <div className="opacity-50 cursor-not-allowed">
                                            <Button isIconOnly size="sm" variant="ghost" isDisabled aria-label="Solo puedes editar tus propios items">
                                    ...
                                    <Button isIconOnly size="sm" variant="ghost" className="text-danger" isDisabled aria-label="Eliminar (No implementado)">
